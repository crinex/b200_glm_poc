...
def main():
    ...
    ap.add_argument("--host", default="172.18.162.57")
    ap.add_argument("--port", type=int, default=30269)
    ap.add_argument("--model", default="", help="empty = use first id from /v1/models")
    ap.add_argument("--gen-dir", default="gen8k")
    ...
    ap.add_argument("--sweep", default="12,64,128,200,256",
    ...
    ap.add_argument("--device", default="H200")
...
def run_once(args, base, jobs, one, conc, out_dir, fmt):
            ...
            try:
                urllib.request.urlopen(
                    urllib.request.Request(
                        base + path, data=b"", headers=request_headers(args.api_key)),
                    timeout=30).read()
    ...
    try:
        code = urllib.request.urlopen(
            urllib.request.Request(
                base + "/health", headers=request_headers(args.api_key)),
            timeout=10).status
