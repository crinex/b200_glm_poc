...
## 6-4. TP=2 · DP=4 · EP=8 — "non-expert 복제 + expert 분산" (서버 E, 2026-09-02)

사용자 제안: MoE 에서 attention/dense 는 DP 로 복제하고 expert 만 EP 로 쪼개면
GPU 당 전문가 메모리가 줄어 KV cache 가 늘지 않나? → **부분적으로 맞았고,
고concurrency 에서 크게 이겼다.**

구성: `--tensor-parallel-size 2 --data-parallel-size 4 --enable-expert-parallel`
(EP = TP×DP = 8). MTP spec=1, 그 외 동일. `--api-server-count 1` 필요
(기본은 DP 수만큼 API 서버를 띄워 기동 타임아웃 발생).

KV cache: rank당 985,984 × 4 = **3.94M tokens (TP8·EP1 의 1.48M 대비 2.67배)**.
"전문가 메모리 절감분이 KV 로 갔다"는 가설이 실측으로 확인됨.

| conc | TP8·EP1 (기준) | TP2·DP4·EP8 | 증감 |
|---|---|---|---|
| 4 | 397 | 266 | -33% |
| 8 | 771 | 489 | -37% |
| 16 | 1,250 | 798 | -36% |
...
### 해석 — concurrency 로 명확히 갈린다

- **저~중 conc (≤32): -33~37% 열세.** conc 요청이 rank 4개로 분산되면 rank당
  요청이 적고, 각 rank 는 TP=2 뿐이라 단일 요청 처리가 느리다 (attention/dense
  가 2장에만 분할 + EP8 all-to-all). 낮은 부하에서 DP 는 병렬 이점이 없다.
- **고 conc (≥128): +14~47% 우세.** rank당 요청이 충분(128÷4=32, 256÷4=64)하고
  KV cache 2.67배 여유가 살아난다. TP8·EP1 은 KV 상주 ~160 에서 포화하지만
  DP4 는 rank당 ~107 × 4 = 총 ~428 을 수용해 conc=256 에서도 큐잉이 적다.
- 손익분기: conc=64 근처 (-3%, 노이즈 내).

### 비용

- **기동 시간 급증**: torch.compile 이 직렬화된다. DP 엔진 4개가 CPU 를 나눠
  `Reducing Torch threads 92→1`, compile range 당 ~107초. 총 기동 ~10분+
  (TP8 은 ~6분). 컴파일 캐시가 생기면 재기동은 빨라질 것.
- 저conc 성능 희생 — 저지연이 필요한 트래픽에는 부적합.

### 결론

**단일 최적 구성은 없다. 트래픽 프로파일로 갈린다:**
- 저지연 / 저~중 부하 → **TP8 · EP1 · MTP** (또는 conc≥128 이면 MTP spec=2)
- 처리량 극대 / 고부하 (conc≥128) → **TP2 · DP4 · EP8 · MTP**

세션 최고 처리량: **TP2·DP4·EP8, conc=256 에서 3,188 tok/s** (TP8·EP1 대비 +47%,
H200 MTP 702 대비 4.5배). 사용자 제안이 만든 결과다.
